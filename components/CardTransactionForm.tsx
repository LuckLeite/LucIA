...
const CardTransactionForm: React.FC<CardTransactionFormProps> = ({ onSubmit, transactionToEdit, onCancelEdit, registries }) => {
  ...
  const resetForm = () => {
    setName('');
    // Seleciona o primeiro cartão cadastrado como padrão
    setCard(registries.length > 0 ? registries[0].name : '');
    setTotalAmount('');
    setInstallments('');
    ...
    <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-md border border-gray-100 dark:border-slate-700">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            <div>
                ...
                <input id="card-name" value={name} onChange={e => setName(e.target.value)} required placeholder="Ex: Geladeira, Celular..."
             ...
             <div>
                <label htmlFor="card-card" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Selecione o Cartão</label>
                <select 
                    id="card-card" 
                    value={card} 
                ...
                >
                    ...
                    {registries.length === 0 && <option value="" disabled>Nenhum cartão cadastrado</option>}
                </select>
                {registries.length === 0 && <p className="mt-1 text-[10px] text-red-500 font-bold uppercase">Cadastre um cartão primeiro no botão "Gerenciar Meus Cartões"</p>}
            </div>
            ...
            <div className="flex justify-end gap-2 pt-2">
                ...
                <button type="submit" disabled={registries.length === 0} className="flex-grow bg-primary-600 text-white font-semibold py-2 px-4 rounded-md hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 disabled:bg-gray-400">
